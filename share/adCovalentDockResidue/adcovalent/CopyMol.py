...
#class DuplicateMolecule(ob.OBMol):
    ...
    def copyAtomsFromMol(self):
        ...
        tot = list(range(self.original.NumAtoms()))
    ...
    def copyAtom(self, atom):
        ...
        self.mol._numbering[atom.GetIdx()] = len(list(self.mol._numbering.keys()))+1
        self.mol._numberingReverse[len(list(self.mol._numbering.keys()))] = atom.GetIdx()
    ...
    def copyBondsFromResidue(self):
        ...
        if self.debug: 
            print("Residue [%s]:"" %d missed bonds" % (res.GetName(), len(missedBonds)), end=' ')
...
if __name__ == "__main__":
    ...
    org = next(pybel.readfile(ext, argv[1]))
