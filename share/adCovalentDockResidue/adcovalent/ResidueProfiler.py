...
class AminoAcidProfiler:
    ...
    def scanResidues(self):
        ...
        """
        for resId, data in list(self.residues.items()):
            resObj = data['obj']
            ...
            if not 'pattern' in self.residues[resId]:
                ...
                #new = dup.getCopy()
                #).__str__().split()[0]
                print("WARNING! Residue [%s] was not recognized" % resId, end=' ')
    ...
    def guessResType(self, resId):
        """ save time"""
        data = list(self.sideChains.items())
    ...
    def getAminoAcidPatterns(self, sort=None):
        ...
        # all other aa's
        for resName in list(self.sideChains.keys()):
    ...
    def getSortedSideChains(self, rType): # XXX OBSOLETE
        ...
        """
        if rType in list(self.sideChains.keys()):
...
if __name__ == '__main__':
    ...
    mol = next(pybel.readfile(ftype, sys.argv[1]))
